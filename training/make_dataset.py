...
for p in sample_parts[:10]:  # 10 parts is enough
    ride_ids.update(pd.read_parquet(p, columns=["ride_id"])["ride_id"].unique())
ride_ids = sorted(ride_ids)
print(f"Found {len(ride_ids)} unique ride_id values")

# ── build entity dataframe (cartesian product) ────────────────────────────
timestamps = pd.date_range(HISTORY_START, HISTORY_END, freq="30min", tz="UTC")
...
training_df = store.get_historical_features(
    ...
    features=[
        ...
        "queue_weather_hourly:temp_f",
        "queue_weather_hourly:precip_prob",
    ],
).to_df()

# ── create 60-min-ahead target ────────────────────────────────────────────
training_df = training_df.sort_values(["ride_id", "event_timestamp"])
training_df["target"] = training_df.groupby("ride_id")["posted_wait"].shift(
    -HORIZON_ROWS
)
